...
$lblTimer = GUICtrlCreateLabel("Timer (s)", 30, 99, 58, 30)
	...
	Switch $nMsg
		...
		Case $btnBegin
			...
			while 1
				...
				Sleep(1000)											;Ver 25 - added so we do not take ss of play screen
				ClickImages($imageArray)							;Clicks the images that will kickoff a test
				ClickStart()
				Sleep($iTimer * 1000)								;Time entered in seconds * milliseconds
...
Func ClickImages($disImageArray)
	...
	While($iFileExists == 1)										;We want to keep looping until a file does not exist
		...
		If $iFileExists == 1 Then
			if $i == 1 Then				;Pause after pressing Play
				Sleep(5000)
			EndIf

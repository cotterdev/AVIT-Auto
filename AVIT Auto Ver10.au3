...
$Form1 = GUICreate("Auto AVIT", 159, 400, 212, 600)
...
Global $yArray[5]
Global $startPic
$xStartPos = 0						;Start has it's own variable because we click this last on it's own
$yStartPos = 0
...
Func GetTargetImageArray()
	For $i = 0 To 7 Step 1											;Loop thru the number of images we have
...
Func ClickImages()
	...
	While($iFileExists == 1)										;We want to keep looping until a file does not exist
		$iFileExists = FileExists($imageArray[$i])
		...
		If $iFileExists == 1 Then
			...
			If $result = 1 Then
				MouseClick('primary', $xArray[$i], $yArray[$i], 1)	;Click on the image
			Else
				MsgBox(16,"Error!", "Image not found: Image# " & $i);We have a file in the directory to search for but did not find it
			EndIf
		EndIf
